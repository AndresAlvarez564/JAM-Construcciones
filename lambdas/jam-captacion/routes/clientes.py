...
def _crear(pk, sk, body, cedula, inmobiliaria_id, proyecto_id):
    ...
    unidades = []
    if unidad_id:
        unidades = [{'unidad_id': unidad_id, 'unidad_nombre': unidad_nombre, 'fecha_vinculacion': fecha_captacion}]
...
def _actualizar_datos(pk, sk, body, existing):
    ...
    # Si viene con unidad nueva, agregarla a la lista si no existe ya
    if body.get('unidad_id'):
        unidad_id, unidad_nombre = _resolver_unidad(body, existing.get('proyecto_id', ''))
        ids_existentes = [u.get('unidad_id') for u in existing.get('unidades', [])]
        if unidad_id not in ids_existentes:
            values[':nueva_unidad'] = [{'unidad_id': unidad_id, 'unidad_nombre': unidad_nombre, 'fecha_vinculacion': _now()}]
            values[':empty_list'] = []
            updates.append('unidades = list_append(if_not_exists(unidades, :empty_list), :nueva_unidad)')
...
def listar_admin(event):
    """GET /admin/clientes — admin ve todos."""
    qs = event.get('queryStringParameters') or {}
    proyecto_id = qs.get('proyecto_id')
    ...
    if proyecto_id:
        ...
        # Scan acotado — solo para admin sin filtros
        result = clientes.scan(Limit=200)
...
def actualizar_admin(event, cedula, proyecto_id):
    ...
    updates, values, names = [], {}, {}
    campos = ['nombres', 'apellidos', 'correo', 'telefono', 'estado_civil',
              'nacionalidad', 'pais_residencia', 'fecha_nacimiento', 'estado']
    for campo in campos:
        if campo in body:
            values[f':{campo}'] = body[campo]
            updates.append(f'#{campo} = :{campo}')
